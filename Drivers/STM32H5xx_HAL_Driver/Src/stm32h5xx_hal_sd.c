...
HAL_StatusTypeDef HAL_SD_Init(SD_HandleTypeDef *hsd)
{
  ...
  printf("%s, %d => call to HAL_SD_InitCard() \r\n", __func__, __LINE__);
  /* Initialize the Card parameters */
  if (HAL_SD_InitCard(hsd) != HAL_OK)
  {
	printf("%s, %d => return HAL_ERROR \r\n", __func__, __LINE__);
    return HAL_ERROR;
  }

  printf("%s, %d => call to HAL_SD_GetCardStatus() \r\n", __func__, __LINE__);
  ...
  printf("%s, %d => call to HAL_SD_ConfigWideBusOperation() \r\n", __func__, __LINE__);
  ...
  printf("%s, %d => return HAL_SD_STATE_READY \r\n", __func__, __LINE__);
  hsd->State = HAL_SD_STATE_READY;
...
HAL_StatusTypeDef HAL_SD_InitCard(SD_HandleTypeDef *hsd)
{
  ...
  printf("%s, %d => call to SD_PowerON() \r\n",__func__, __LINE__);
  errorstate = SD_PowerON(hsd);
  if (errorstate != HAL_SD_ERROR_NONE)
  {
    hsd->State = HAL_SD_STATE_READY;
    hsd->ErrorCode |= errorstate;
    printf("%s(), %d => errorstate=%x (return HAL_ERROR) \r\n",__func__, __LINE__, errorstate);
    return HAL_ERROR;
  }

  /* Card initialization */
  printf("%s, %d => call to SD_InitCard() \r\n",__func__, __LINE__);
  ...
  printf("%s, %d => call to SDMMC_CmdBlockLength() \r\n",__func__, __LINE__);
...
static uint32_t SD_PowerON(SD_HandleTypeDef *hsd)
{
  ...
  printf("%s, %d => SdCard.CardVersion=%d\r\n", __func__, __LINE__,hsd->SdCard.CardVersion);

  if (hsd->SdCard.CardVersion == CARD_V2_X)
  {
    /* SEND CMD55 APP_CMD with RCA as 0 */
    /* CMD55 (RCA=0)：告訴卡「下一個是 APP_CMD」 */
    errorstate = SDMMC_CmdAppCommand(hsd->Instance, 0);
    if (errorstate != HAL_SD_ERROR_NONE)
    {
        printf("%s, %d => N○ Card detected (HAL_SD_ERROR_UNSUPPORTED_FEATURE)\r\n", __func__, __LINE__);
    	return HAL_SD_ERROR_UNSUPPORTED_FEATURE;
    }
  }
  ...
  while ((count < SDMMC_MAX_VOLT_TRIAL) && (validvoltage == 0U))
  {
    ...
    printf("%s[%d] : validvoltage=%d \r\n", __func__, __LINE__,validvoltage );
